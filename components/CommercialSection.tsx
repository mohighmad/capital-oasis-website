...
export function CommercialSection() {
  const { t } = useLanguage();
  const commercialLightboxItems = commercialProjects.map((project, index) => ({
  ...
  const commercialLightbox = useImageLightbox(commercialLightboxItems);
    ...
    <section id="commercial" className="section-pad wood-grain bg-deep-green text-white">
      <div className="shell grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          ...
          <div className="image-reveal relative col-span-2 aspect-[4/3] overflow-hidden rounded-3xl border border-white/10 sm:aspect-[16/10]">
            ...
            <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-6">
              <p className="text-sm font-bold text-gold">
                {t({
                  en: "Stores · Showrooms · Cafes · Offices",
                  ar: "محلات · معارض · مقاهٍ · مكاتب",
                })}
              </p>
            </div>
            <ImageLightboxTrigger
              label={t({
                en: "View commercial image 1",
                ar: "عرض صورة الأعمال التجارية 1",
              })}
              onOpen={(trigger) =>
                commercialLightbox.openAtIndex(0, trigger)
              }
            />
          </div>
          {commercialProjects.slice(1).map((project, index) => (
            <div
              key={project.image}
            ...
            >
              ...
              <ImageLightboxTrigger
                label={t({
                  en: `View commercial image ${index + 2}`,
                  ar: `عرض صورة الأعمال التجارية ${index + 2}`,
                })}
                onOpen={(trigger) =>
                  commercialLightbox.openAtIndex(index + 1, trigger)
                }
              />
            </div>
          ))}
        ...
        <div>
          ...
          <p className="eyebrow mt-7 mb-5 text-gold">
            {t({ en: "Commercial interiors", ar: "المساحات التجارية" })}
          </p>
        ...
        <ImageModal
          items={commercialLightboxItems}
          activeIndex={commercialLightbox.activeIndex}
          canGoNext={commercialLightbox.canGoNext}
          canGoPrevious={commercialLightbox.canGoPrevious}
          onClose={commercialLightbox.close}
          onNext={commercialLightbox.goToNext}
          onPrevious={commercialLightbox.goToPrevious}
        />
      </div>
    </section>
  );
